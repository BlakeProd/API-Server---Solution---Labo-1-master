...
class Repository {
        ...
        if (params) {
            ...
            Object.keys(params).forEach(function (paramName) {
                if (paramName == "sort") {
                    ...
                    // Saisir la valeur du champ à filtrer
                    if(paramName in model){
                        let value = params[paramName];
                        if(value){
                            searchKeys.push({key: paramName, value});
                        }
                    }   
                }
            });
            ...
            if(sortKeys.length > 0)
            {
                ...
                {
                    filteredAndSortedObjects = this.SortObject(filteredAndSortedObjects,sortKeys);
                }
                else
                {
                    filteredAndSortedObjects = this.SortObject(objectsList,sortKeys);
                }
    ...
    SortObject(objetList,sortKey)
    {
        this.sortFields = sortKey;
        let newTable = [...objetList].sort((a,b) =>
        {
    ...
    compare(value, sortValue) {
        let fieldIndex = 0;
        let max = this.sortFields.length;
        do {
            let result = 0;
            if (this.sortFields[fieldIndex].asc)
                result = this.innerCompare(value[this.sortFields[fieldIndex].key], sortValue[this.sortFields[fieldIndex].key]);
            else
                result = this.innerCompare(sortValue[this.sortFields[fieldIndex].key], value[this.sortFields[fieldIndex].key]);
            if (result == 0)
                fieldIndex++;
            else
                return result;
